...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('materials', (projectId) => ({
    ...
    selected: new Set(),
    filterOptions: { stations: [], purchasing_groups: [], suppliers: [] },

    filters: { search:'', po_number:'', status:'', supplier:'', station_no:'', purchasing_group:'', is_focus:'', overdue:false, no_eta:false },
    ...
    resetFilter()  { this.filters = { search:'', po_number:'', status:'', supplier:'', station_no:'', purchasing_group:'', is_focus:'', overdue:false, no_eta:false }; this.applyFilter(); },
    ...
    isOverdue(item) {
      return item.current_eta && item.status === 'open' && new Date(item.current_eta) < new Date();
    },
    noEta(item)  { return (!item.current_eta || item.current_eta === '') && item.status === 'open'; },
    statusBadge(s) { return ({open:'badge-open',delivered:'badge-delivered',cancelled:'badge-cancelled',on_hold:'badge-on_hold'})[s] || 'badge-open'; },
